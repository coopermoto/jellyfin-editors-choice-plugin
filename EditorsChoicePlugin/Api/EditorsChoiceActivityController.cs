            ...
            if (_config.Mode == "FAVOURITES")
            {
                ...
                else
                {
                    ...
                    query = new InternalItemsQuery(activeUser)
                    {
                        ...
                        IsUnaired = _config.ShowUnaired ? null : false,
                    };
    ...
    private List<BaseItem> PrepareResult(InternalItemsQuery query, Jellyfin.Data.Entities.User? activeUser)
    {
        ...
        for (int i = 0; i < _config.RandomMediaCount && i < max; i++)
        {
            ...
            if (shiftItem.IsVisible(activeUser) &&
                !result.Contains(shiftItem) &&
                inFilteredLibrary &&
                !(shiftItem.IsPlayed(activeUser) && !_config.ShowPlayed) &&
                !(shiftItem.IsUnaired && !_config.ShowUnaired) &&
                shiftItem.HasImage(MediaBrowser.Model.Entities.ImageType.Backdrop))
            {
